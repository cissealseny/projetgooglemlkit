...
class ChatView(APIView):
    ...
    def post(self, request):
        ...
        if conversation_id:
            ...
            conversation = Conversation.objects.create(
                user=request.user, title=message_content[:50], model=model
            )
    ...
    def _maybe_collect_nearby_places(
        self, request, message, latitude, longitude, radius_meters, user
    ):
                ...
                places.append(
                    {
                        ...
                        "photo_proxy_url": self._build_place_photo_proxy_url(
                            request, metadata.get("photo_reference"), max_width=900
                        ),
                        "thumbnail_proxy_url": self._build_place_photo_proxy_url(
                            request, metadata.get("photo_reference"), max_width=480
                        ),
    ...
    def _extract_place_categories(self, message):
        ...
        if (
            "fast food" in text_lower
            or "fastfood" in text_lower
            or "snack" in text_lower
        ):
...
class TextGenerationView(APIView):
    ...
    def post(self, request):
        ...
        result = generative_service.generate_text(
