...
class NLPService:
    ...
    def __init__(self):
        ...
        self.default_provider = (
            os.getenv("NLP_PROVIDER", "auto").strip().lower() or "auto"
        )
    ...
    def detect_language(self, text):
        ...
        try:
            ...
            return {
                ...
                "probabilities": [
                    {
                        "language": str(p).split(":")[0],
                        "probability": float(str(p).split(":")[1]),
                    }
    ...
    def translate_text(
        self, text, source_language="auto", target_language="en", provider="auto"
    ):
        """
        ...
        try:
            ...
            if self._should_use_google(resolved):
                ...
                result = self._transformers_translate(
    ...
    def summarize_text(self, text, max_length=150, min_length=50, provider="auto"):
        ...
        if self._should_use_google(resolved):
            return {
                ...
                + ("..." if len(text) > min_length else ""),
                "original_length": len(text),
                "summary_length": min(len(text), min_length + 3),
                "compression_ratio": (
                    (min(len(text), min_length + 3) / len(text)) if len(text) else 1
                ),
    ...
    def _classify_with_categories_fast(self, text, categories):
        ...
        for raw_category in categories:
            ...
            substring_bonus = (
                1.0 if category_norm and category_norm in text_norm else 0.0
            )
        ...
        if max_raw <= 0:
            ...
            categories_out = [
                {"label": item["label"], "score": uniform} for item in scored
            ]
        else:
            ...
            categories_out = [
                {
                    "label": item["label"],
                    "score": (item["raw_score"] / total) if total else 0.0,
                }
                for item in scored
            ]
        ...
        top = (
            categories_out[0] if categories_out else {"label": "UNKNOWN", "score": 0.0}
        )
