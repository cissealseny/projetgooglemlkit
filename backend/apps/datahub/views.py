...
class GooglePlacePhotoProxyView(APIView):
    ...
    def get(self, request):
        ...
        if not photo_reference:
            return Response({'success': False, 'error': 'photo_reference requis'}, status=400)
        ...
        if not data_collection_service.google_maps_api_key:
            return Response({'success': False, 'error': 'GOOGLE_MAPS_API_KEY non configuree'}, status=503)
        ...
        if cached:
            response = HttpResponse(cached.get('content', b''), content_type=cached.get('content_type', 'image/jpeg'))
