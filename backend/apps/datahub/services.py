    ...
    def collect_facebook(self, page_id, limit=25):
        ...
        try:
            ...
            if response.status_code >= 400 or payload.get("error"):
                err = payload.get("error", {}).get(
                    "message", "Erreur Facebook Graph API"
                )
                return {
                    "success": False,
                    "error": err,
                ...
                records.append(
                    {
                        ...
                        "metadata": {
                            ...
                            "comments_count": self._safe_int(
                                ((post.get("comments") or {}).get("summary") or {}).get(
                                    "total_count"
                                )
                            ),
                            "shares_count": self._safe_int(
                                (post.get("shares") or {}).get("count")
                            ),
                        },
                    }
                )
        ...
        try:
            ...
            if status not in ("OK", "ZERO_RESULTS"):
                return self._collect_osm_places(
                    ...
                    max_results=max_results,
                    start_time=start_time,
                    fallback_reason=payload.get("error_message")
                    or f"Google Places status: {status}",
                )
            ...
            for item in payload.get("results", [])[:capped_results]:
                ...
                photo_reference = ((item.get("photos") or [{}])[0] or {}).get(
                    "photo_reference"
                )
                ...
                metadata = {
                    ...
                    "user_ratings_total": self._safe_int(
